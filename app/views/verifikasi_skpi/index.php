            ...
            <div class="bg-white px-3">
                <form action="">
                    ...
                    <div>
                        ...
                        <input type="radio" name="status" id="belumVerifikasi" value="Belum Disetujui"> Belum Verifikasi
                        <input type="radio" name="status" id="sudahVerifikasi" value="Sudah Disetujui"> Sudah Verifikasi <br>
                    </div>
                </form>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr class=" border-b border-gray-300">
                            ...
                            <td class="text-center">
                                <label ><?= date("d F Y", strtotime($mhs['tanggal_pelaksanaan'])) ?></label><br>
                                <?php if ($mhs['tanggal_berakhir'] != null) { ?>
                                <label >s.d.</label><br>
                                <label ><?= date("d F Y", strtotime($mhs['tanggal_berakhir'])) ?></label>
                                <?php } ?>
                            </td>
                            <td>
                                <?php if ($mhs['nama_butir'] != null) : ?>
                                <label for="">Level/Tingkat : <?= $mhs['nama_butir'] ?></label><br>
                                <?php endif; ?>
                                <?php if ($mhs['nama_sub_butir'] != null && $mhs['id_kategori'] == 1) { ?>
                                <label for="">Juara : <?= $mhs['nama_sub_butir'] ?> </label><br>
                                <?php } elseif ($mhs['nama_sub_butir'] != null && $mhs['id_kategori'] != 1) {?>
                                <label for="">Kepesertaan : <?= $mhs['nama_sub_butir'] ?> </label><br>
                                <?php } ?>
